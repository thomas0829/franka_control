...
class DR_CREPS_PE(BlackBoxOptimization):
	...
	def _update(self, Jep, theta):
		...
		# REPS
		eta_start = np.ones(1)
		res = minimize(DR_CREPS_PE._dual_function, eta_start,
					   jac=DR_CREPS_PE._dual_function_diff,
		...
		# constrained WMLE
		if type(self.distribution) is GaussianDistributionGDR:
			self.distribution.con_wmle_gdr(theta, d, self._eps(), self._kappa(), self.top_k_corr)
		elif type(self.distribution) is GaussianDiagonalDistribution:
			self.distribution.con_wmle(theta, d, self._eps(), self._kappa(), self.top_k_corr)
