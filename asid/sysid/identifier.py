...
class CEM():
    def __init__(self, distribution, weighted=False, elite_frac=0.1, keep_elite=False) -> None:
    ...
    def _update(self, Jep, theta):
        ...
        if self.keep_elite and self.elite_thetas is not None and self.elite_Jeps is not None:
        ...
        self.dist.mle(self.elite_thetas, weights=self.elite_Jeps if self.weighted else None)
...
class SysIdentifier():
    def __init__(
        ...
        if dist == "diag":
            if algo_name == "cem":
                self.distribution = GaussianDiagonalDistribution(
                    ...
                    std = np.ones(zeta_dim) * sigma_init
                )
            else:
                print(f"Warning: Diagonal distribution not supported for {algo_name} using Cholesky distribution instead")
        ...
        if algo_name == "reps":
            ...
            self.algo = ConstrainedREPS(None, self.distribution, None, eps=eps, kappa=kappa)
        elif algo_name == "drcreps":
            self.algo = DR_CREPS_PE(None, self.distribution, None, eps=eps, kappa=kappa, lambd=lambd, k=k, C="PCC")
        elif algo_name == "cem":
            self.algo = CEM(self.distribution, elite_frac=elite_frac, weighted=wmle, keep_elite=keep_elite)
    ...
    def identify(self, real_obs, real_acts, real_zeta, envs):
        ...
        for i in trange(self.n_epochs, disable=not self.verbose):
            ...
            if self.verbose:
                print(f"Epoch {i} - mean {self.distribution._mu} - cholesky sigma {self.distribution._std if type(self.distribution) == GaussianDiagonalDistribution else self.distribution._chol_sigma} - fitness: {np.mean(fit)}")
                print(f"best fitness: {self.best_J} - best params: {self.best_params} - real param: {real_zeta}")
        ...
        return np.mean(fit), self.distribution._mu, self.distribution._std if type(self.distribution) == GaussianDiagonalDistribution else self.distribution._chol_sigma
    
    def fitness(self, params, real_obs, real_acts, envs, render=False):

        sim_obs = None
        # for i in trange(self.population_size):
        for i in trange(self.population_size // self.num_workers, disable= not self.verbose):
            ...
            if self.rnd_env_seed:
                envs.seed(self.seed)
                envs.set_parameters(params[i*self.num_workers:(i+1)*self.num_workers])
            else:
            ...
            if self.logger is not None and render and i == 0:
                imageio.mimsave(f"sim_{np.around(params[0][0], 2)}.gif", np.stack(frames)[:,0], duration=3)
            ...
            sim_obs = sim_obs_tmp if sim_obs is None else np.concatenate((sim_obs, sim_obs_tmp), axis=1)
    ...
    def compute_reward(self, obs_real, obs_sim):
        ...
        # obj pose
        return - np.linalg.norm(obs_real[...,-7:] - obs_sim.astype(np.float32)[...,-7:], axis=(0,2))
    
    def sample(self):
        candidates = None
        for _ in range(self.population_size):
            sample = self.distribution.sample()[np.newaxis]
            candidates = sample if candidates is None else np.concatenate([candidates, sample], axis=0)
    ...
    def log_distribution(self):
        ...
        sig = self.distribution._std if type(self.distribution) == GaussianDiagonalDistribution else self.distribution._chol_sigma
    ...
    def save_zeta(self, real_obs, filename):
        ...
        param_dict = {
            ...
            "sigma": self.distribution._std if type(self.distribution) == GaussianDiagonalDistribution else self.distribution._chol_sigma,
        }
