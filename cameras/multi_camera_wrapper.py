...
class MultiCameraWrapper:
    ...
    def get_aruco_poses(self, marker_size=0.15, aruco_dict=cv2.aruco.DICT_6X6_50, steps=1, verbose=False):
        tvecs = []
        rotation_matrices = []
        for camera in self._all_cameras:
            tvec, rotation_matrix = self._get_aruco_pose(camera, marker_size=marker_size, aruco_dict=aruco_dict, steps=steps, verbose=verbose)
            tvecs.append(tvec)
            rotation_matrices.append(rotation_matrix)
        return tvecs, rotation_matrices
    
    def _get_aruco_pose(self, camera, marker_size=0.15, aruco_dict=cv2.aruco.DICT_6X6_50, steps=1, verbose=False):
        ...
        for _ in range(steps):
            ...
            if ids is not None:
                ...
                rvec, tvec, _ = self._estimatePoseSingleMarkers(corners[0], marker_size=marker_size, mtx=camera_matrix, distortion=dist_coeff)
                # same as: (opencv-contrib-python 4.6.0.66)
                ...
                if verbose:
                    ...
                    img_rend = cv2.drawFrameAxes(img_rend, camera_matrix, dist_coeff, rvec[0], tvec[0], length_of_axis)
                    plt.imshow(img_rend)
