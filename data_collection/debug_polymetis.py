...
def run_experiment(cfg):
    ...
    while True:
        ...
        while True:
            ...
            if info["success"]: # press A to break the loop
        ...
        for j in tqdm(
            ...
            # if not press A, B, or tripper, wait for input
            ...
            if info["movement_enabled"]:
                ...
                delta_act = env._robot._ik_solver.cartesian_velocity_to_delta(
                    vel_act
                )
                
                # print("delta_act", delta_act)
                end_time = time.time()
                
                average_times.append(end_time - start_time)
        ...
        # print("Average time per action(ms): ", np.mean(average_times) * 1000)
        print("Average time per action(ms): ", np.mean(average_times) * 1000)
