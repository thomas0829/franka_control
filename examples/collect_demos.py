...
if __name__ == "__main__":
    ...
    # hardware
    parser.add_argument("--dof", type=int, default=3, choices=[3, 4, 6])
    parser.add_argument(
    ...
    for i in range(args.episodes):
        ...
        for j in tqdm(
            ...
            if info["movement_enabled"]:
                ...
                delta_gripper = env._robot._ik_solver.gripper_velocity_to_delta(
                    vel_act[-1:]
                )
                # prepare act
                if args.dof == 3:
                ...
                # tmp_vel_act = env._robot._ik_solver.cartesian_delta_to_joint_velocity(delta_act, env._robot.get_robot_state()[0])
                # tmp_act = np.concatenate((tmp_vel_act, vel_act[-1:]))
        ...
        save_path = os.path.join(
            save_dir, f"traj_{datetime.datetime.now().strftime('%m_%d_%Y-%H_%M_%S')}.gz"
        )
