...
def get_shape_metadata_from_dataset(dataset_path, action_keys, all_obs_keys=None, language_conditioned=False, verbose=False):
    """
    ...
    Args:
        ...
        language_conditioned (bool): hack for language conditioning - get embedding dimension separately
            instead of from dataset
    ...
    if language_conditioned:
        assert LangUtils.LANG_OBS_KEY in all_obs_keys

    for k in sorted(all_obs_keys):
        if language_conditioned and (k == LangUtils.LANG_OBS_KEY):
            initial_shape = (LangUtils.get_lang_emb("hi").shape[0],)
        else:
...
def get_motion_planner_metadata_from_dataset(dataset_path):
    """
    Retrieves metadata needed for motion planner rollout wrapper from dataset.

    Args:
        dataset_path (str): path to dataset

    Returns:
        task_spec (mimicgen TaskSpec instance): task spec object, which describes the sequence
                of object-centric stages for this task. Each stage will consist of a motion planner
                segment, and a policy segment

        cached_motion_planner_targets (dict): dictionary with a key per stage (and special "final" key
            for final stage). Each stage key is mapped to a list of 4x4 poses, where each pose is a
            target eef pose relative to the object in that stage (or an absolute eef pose if there is
            no reference object in that stage). These will typically be extracted from a set of human
            demonstrations and cached in an hdf5.
    """
    dataset_path = os.path.expandvars(os.path.expanduser(dataset_path))
    f = h5py.File(dataset_path, "r")
    task_spec = json.loads(f["data"].attrs["mg_task_spec"])
    cached_motion_planner_targets = None
    if "mg_motion_plan_targets" in f["data"].attrs:
        cached_motion_planner_targets = json.loads(f["data"].attrs["mg_motion_plan_targets"])
    else:
        LogUtils.log_warning("mg_motion_plan_targets not found in dataset!")
    f.close()

    task_spec_obj, cached_motion_planner_targets_np = get_motion_planner_metadata_from_json_dicts(
        task_spec_json=task_spec, cached_motion_planner_targets_json=cached_motion_planner_targets,
    )
    return task_spec_obj, cached_motion_planner_targets_np


def get_motion_planner_metadata_from_json_dicts(task_spec_json=None, cached_motion_planner_targets_json=None):
    """
    Helper function to convert json metadata to objects.
    """
    task_spec_obj = None
    cached_motion_planner_targets_np = None

    if task_spec_json is not None:
        # convert json dict to task spec instance
        from mimicgen.utils.traj_utils import TaskSpec
        task_spec_obj = TaskSpec.from_json(json_dict=task_spec_json)

    if cached_motion_planner_targets_json is not None:
        # convert each pose in list of lists to np arrays
        cached_motion_planner_targets_np = {
            stage_key : [
                np.array(pose)
                for pose in cached_motion_planner_targets_json[stage_key]
            ]
            for stage_key in cached_motion_planner_targets_json
        }
...
def policy_from_checkpoint(device=None, ckpt_path=None, ckpt_dict=None, verbose=False):
    ...
    # ensure language conditioning will take place
    if config.observation.get("language_conditioned", False):
        # assert LangUtils.LANG_OBS_KEY in config.observation.modalities.obs.low_dim
        assert LangUtils.LANG_OBS_KEY in config.observation.modalities.obs.lang
