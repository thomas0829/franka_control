...
def load_data_for_training(config, obs_keys):
    ...
    if config.experiment.validate:
        ...
        print(train_demo_keys, valid_demo_keys)
...
def data_loaders_to_iter(data_loaders):
    """
    Convert dataloader list into iterator list
    """
    data_loader_iters = []
    for dataloader in data_loaders:
        data_loader_iters.append(iter(dataloader))
    return data_loader_iters


def get_next_batch(data_loader_iters):
    """
    Generate batch from multiple dataloaders
    """
    batch = []
    # load according to batch weighting and concatenate 
    for data_loader_iter in data_loader_iters:
        batch.append(next(data_loader_iter))

    # concatenate batches (keys)
    for i in range(len(batch)-1):
        for k in batch[i].keys():
            # if additional key layer, iterate again
            if type(batch[0][k]) == torch.Tensor:
                batch[0][k] = torch.cat((batch[0][k], batch[i+1][k]), dim=0)
            else:
                for l in batch[i][k].keys():
                    batch[0][k][l] = torch.cat((batch[0][k][l], batch[i+1][k][l]), dim=0)
    return batch[0]


def run_epoch_mt(model, data_loaders, epoch, validate=False, num_steps=None, obs_normalization_stats=None):
    """
    ...
    Args:
        ...
        data_loaders list(DataLoader instance): data loader that will be used to serve batches of data
            to the model
