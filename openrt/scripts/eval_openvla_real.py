...
@hydra.main(
    ...
    # config_path="../../configs/", config_name="eval_openvla_real_bimanual", version_base="1.1"
...
def run_experiment(cfg):
    ...
    demo_dir = "/home/prior/yuquand/data/lift_box/npy/lift_box_1/train/episode_7.npy"
    camera_name = "215122252864"
    url = cfg.url
    print("[INFO] mode: ", mode)
    print("[INFO] url: ", url)
    print("[INFO] msg: ", cfg.msg)
    print("[INFO] camera_name: ", camera_name)
    if mode == "replay":
        ...
        while True:
            ...
            path_name = f"/home/robots/yuquand/outputs/molmoact_franka_kitchen_eval/openvla/{task_name}"
            ...
            try:
                for i in range(cfg.traj_length): 
                    ...
                    img = image_preprocessing(obs[camera_name+"_rgb"])
                    img = obs[camera_name+"_rgb"]
                    msg = cfg.msg

                    # sanity check            
                    # from PIL import Image
                    Image.fromarray(img).save("/home/robots/tmp/groundvla_eval.png")
                    ...
                    # left_action, right_action = act[:7], act[7:]


                    # print(f"[OpenVLA Client] Received Action (deltas): {act}")
                    
                    # step
                    # next_obs, reward, done, info = env.step_all(left_action, right_action)
        ...
        for i in range(len(demo)):
            print(f'Step: {i} | lang: {cfg.msg}')
            start = time.time()
            img = image_preprocessing(dic[camera_name+"_rgb"][i])
            msg = cfg.msg
