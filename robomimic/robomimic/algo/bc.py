...
def algo_config_to_class(algo_config):
    ...
    gaussian_enabled = ("gaussian" in algo_config
                        and algo_config.gaussian.enabled)
    ...
    transformer_enabled = ("transformer"
                           in algo_config) and algo_config.transformer.enabled
...
class BC(PolicyAlgo):
    ...
    def process_batch_for_training(self, batch):
        ...
        input_batch["goal_obs"] = batch.get("goal_obs",
                                            None)  # goals may not be present
        input_batch["actions"] = batch["actions"][:, 0, :]
        # we move to device first before float conversion because image observation modalities will be uint8 -
        # this minimizes the amount of data transferred to GPU
        return TensorUtils.to_float(
            TensorUtils.to_device(input_batch, self.device))
    ...
    def train_on_batch(self, batch, epoch, validate=False):
        ...
        with TorchUtils.maybe_no_grad(no_grad=validate):
            info = super(BC, self).train_on_batch(batch,
                                                  epoch,
                                                  validate=validate)
    ...
    def _forward_training(self, batch):
        ...
        actions = self.nets["policy"](obs_dict=batch["obs"],
    ...
    def _compute_losses(self, predictions, batch):
        ...
        losses["l2_loss"] = nn.MSELoss()(actions, a_target) * 100 
        # + nn.MSELoss(
        # )(actions[:, 2], a_target[:, 2]) * 200
        # losses["l2_loss"] = torch.mean(
        #     torch.norm(abs(actions[:, [0, 1, 2]] - a_target[:, [0, 1, 2]]),
        #                dim=1) * 100 +
        #     torch.norm(actions[:, 3:6] - a_target[:, 3:6], dim=1)
        # ) + torch.mean(abs(actions[:, 2] - a_target[:, 2])) * 500 + torch.mean(
        #     abs(actions[:, -1] - (a_target[:, -1] + 1) /
        #         2)) * 500  #nn.MSELoss()(actions, a_target)

        losses["l1_loss"] = nn.SmoothL1Loss()(actions, a_target)
        # cosine direction loss on eef delta position
        losses["cos_loss"] = LossUtils.cosine_loss(actions[..., :3],
                                                   a_target[..., :3]) * 100
...
class BC_VAE(BC):
    ...
    def train_on_batch(self, batch, epoch, validate=False):
        ...
        if self.algo_config.vae.prior.use_categorical:
            ...
            temperature = max(temperature,
                              self.algo_config.vae.prior.categorical_min_temp)
            self.nets["policy"].set_gumbel_temperature(temperature)
        return super(BC_VAE, self).train_on_batch(batch,
                                                  epoch,
                                                  validate=validate)
    ...
    def log_info(self, info):
        ...
        if self.algo_config.vae.prior.use_categorical:
            log["Gumbel_Temperature"] = self.nets[
                "policy"].get_gumbel_temperature()
        else:
            log["Encoder_Variance"] = info["predictions"][
                "encoder_variance"].mean().item()
        if "policy_grad_norms" in info:
            log["Policy_Grad_Norms"] = info["policy_grad_norms"]
        return log
...
class BC_RNN(BC):
    ...
    def process_batch_for_training(self, batch):
        ...
        if self._rnn_is_open_loop:
            ...
            input_batch["obs"] = TensorUtils.unsqueeze_expand_at(obs_seq_start,
                                                                 size=n_steps,
                                                                 dim=1)
    ...
    def get_action(self, obs_dict, goal_dict=None):
        ...
        if self._rnn_hidden_state is None or self._rnn_counter % self._rnn_horizon == 0:
            ...
            if self._rnn_is_open_loop:
                ...
                self._open_loop_obs = TensorUtils.clone(
                    TensorUtils.detach(obs_dict))
...
class BC_Transformer(BC):
    ...
    def _create_networks(self):
        ...
        self.nets["policy"] = PolicyNets.TransformerActorNetwork(
            ...
            encoder_kwargs=ObsUtils.obs_encoder_kwargs_from_config(
                self.obs_config.encoder),
            **BaseNets.transformer_args_from_config(
    ...
    def process_batch_for_training(self, batch):
        ...
        input_batch = TensorUtils.to_device(TensorUtils.to_float(input_batch),
                                            self.device)
        return input_batch
    ...
    def _forward_training(self, batch, epoch=None):
        ...
        TensorUtils.assert_size_at_dim(
            ...
            dim=1,
            msg=
            "Error: expect temporal dimension of obs batch to match transformer context length {}"
    ...
    def get_action(self, obs_dict, goal_dict=None):
        ...
        return self.nets["policy"](obs_dict, actions=None,
                                   goal_dict=goal_dict)[:, -1, :]
