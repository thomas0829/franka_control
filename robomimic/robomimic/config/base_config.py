...
def config_factory(algo_name, dic=None):
    ...
    if algo_name not in REGISTERED_CONFIGS:
        raise Exception(
            "Config for algo name {} not found. Make sure it is a registered config among: {}"
            .format(algo_name, ', '.join(REGISTERED_CONFIGS)))
...
class BaseConfig(Config):
    ...
    def experiment_config(self):
        ...
        # add language
        self.experiment.env_lang = None                             # if using language conditioning, should specify language goal
        self.experiment.additional_envs_langs = None                # if using language conditioning with more envs, should specify language goal for each
        ...
        self.experiment.rollout.n = 1  # number of rollouts per evaluation
    ...
    def train_config(self):
        ...
        self.train.data_co = None                                      
        self.train.data_weights = None                                      
        ...
        # Checkpoint directory for fine-tuning a model
        self.train.ckpt_dir = None
    ...
    def observation_config(self):
        ...
        self.observation.modalities.obs.low_dim = [
            ...
            "cube_pos", "cube_quat", "target_placement_pos",
        ...
        self.observation.encoder.low_dim.core_kwargs = Config(
        ...
        self.observation.encoder.low_dim.obs_randomizer_kwargs = Config(
        )  # No kwargs by default
        ...
        self.observation.encoder.rgb.core_kwargs = Config(
        ...
        self.observation.encoder.rgb.obs_randomizer_kwargs = Config(
        )  # See models/obs_core.py for important kwargs to set and defaults used
        ...
        self.observation.encoder.scan.core_kwargs = Config(
        )  # See models/obs_core.py for important kwargs to set and defaults used
        self.observation.encoder.scan.core_kwargs.do_not_lock_keys()
        
        # =============== Lang default encoder (no encoder) ===============
        self.observation.encoder.lang = deepcopy(self.observation.encoder.low_dim)

        # Whether to use language conditioning (will add a language embedding as a low-dim observation)
        self.observation.language_conditioned = False
    ...
    def meta_config(self):
        ...
        self.meta.hp_values = [
        ]  # values corresponding to keys in hyperparam sweep

    @property
    def use_goals(self):
        # whether the agent is goal-conditioned
        return len([
            obs_key for modality in self.observation.modalities.goal.values()
        ...
        return sorted(
            tuple(
                set([
                    ...
                    ] for modality in group for obs_key in modality
                ])))
