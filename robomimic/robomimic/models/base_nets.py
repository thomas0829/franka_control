...
class CLIPModelVisionModel(ConvBase):
    """
    A ResNet18 block that can be used to process input images and uses FiLM for language conditioning.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            input_coord_conv (bool): if True, use a coordinate convolution for the first layer
                (a convolution where input channels are modified to encode spatial pixel location)
        """
        super(CLIPModelVisionModel, self).__init__()
        ...
        self.processor = CLIPProcessor.from_pretrained(
            "openai/clip-vit-large-patch14-336").image_processor

        self.vision_transformer = model.vision_model
        self.visual_projection_layer = model.visual_projection

        # cut the last fc layer
        self._input_coord_conv = input_coord_conv
        self._input_channel = input_channel

        self._output_channels = model.visual_projection.out_features
        self.device = device
        ...
        if freeze:
            ...
            # Freeze the parameters in the visual_projection
            for param in self.visual_projection_layer.parameters():
                param.requires_grad = False
    ...
    def forward(self, inputs):
        ...
        vision_outputs = self.vision_transformer(pixel_values=inputs.to(self.device))
        pooled_output = vision_outputs[1]  # pooled_outpu
        image_features = self.visual_projection_layer(pooled_output)
...
class CLIPVision(ConvBase):
    """
    A CLIP block that can be used to process input images.
    """
    def __init__(
        self,
        clip_model_class = "openai/clip-vit-base-patch16",
        # clip_model_class = "openai/clip-vit-large-patch14-336",
        freeze=False,
    ):
        """
        Args:
            clip_model_class (str): select one of the CLIP pretrained models
            freeze (bool): if True, use a frozen CLIP pretrained model.
        """
        super(CLIPVision, self).__init__()
        
        # pip install transformers
        # from transformers import CLIPImageProcessor
        # self.image_processor = CLIPImageProcessor.from_pretrained(clip_model_class)
        
        from transformers import CLIPVisionModel
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.vision_model = CLIPVisionModel.from_pretrained(clip_model_class).to(device)
        self.vision_model.requires_grad_(not freeze)
    ...
    def forward(self, inputs):
        ...
        # # only process when using a CLIP trained on different resolution than input img
        # # image_processor torch -> numpy -> torch
        # device = inputs.device
        # processed_inputs = self.image_processor(images=inputs, return_tensors="pt", do_rescale=False, do_normalize=False).to(device)

        # # LLaVA uses 2nd to last layer states
        # mm_vision_select_layer = -2
        # features = self.vision_model(**processed_inputs, output_hidden_states=True).hidden_states[mm_vision_select_layer]
        features = self.vision_model(pixel_values=inputs)
