...
class ObservationEncoder(Module):
    ...
    def _get_vis_lang_info(self):
        ...
        if (len(rgb_inds_need_lang_cond) > 0) or LangUtils.LANG_VIS_MULT_ENABLED:
    ...
    def forward(self, obs_dict):
        ...
        for ind, k in enumerate(self.obs_shapes):
            ...
            if LangUtils.LANG_COND_ENABLED and (not include_lang_feat) and (ind in lang_inds):
                continue
            x = obs_dict[k]
            ...
            if self.obs_nets[k] is not None:
                if LangUtils.LANG_COND_ENABLED and (ind in rgb_inds_need_lang_cond):
        ...
        if LangUtils.LANG_COND_ENABLED and LangUtils.LANG_VIS_MULT_ENABLED:
            # multiply the vision feats by the lang feats and then remove the lang feats from the list 
            if len(lang_inds) > 0:
                new_feats = []
                for ind, k in enumerate(self.obs_shapes):
                    if ind in lang_inds:
                        continue
                    if ind in rgb_inds:
                        new_feats.append(feats[ind] * feats[lang_inds[0]])
                    else:
                        new_feats.append(feats[ind])
                feats = new_feats
...
class MIMO_MLP(Module):
    ...
    def __init__(
        ...
        if LangUtils.LANG_COND_ENABLED:
            mlp_input_dim -= self.input_obs_group_shapes["obs"][LangUtils.LANG_OBS_KEY][0]
    ...
    def forward(self, **inputs):
        ...
        # inputs["obs"]["object"]  = inputs["obs"]["object"][:,:10]
...
class RNN_MIMO_MLP(Module):
    ...
    def __init__(
        ...
        # flat encoder output dimension
        if LangUtils.LANG_COND_ENABLED:
            rnn_input_dim -= self.input_obs_group_shapes["obs"][LangUtils.LANG_OBS_KEY][0]
