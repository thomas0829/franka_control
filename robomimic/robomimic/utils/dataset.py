...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        #     # ensure embedded language is provided in the hdf5 dataset
        #     assert self.lang_key is not None
        #     # self._lang_emb = self.obs_keys[LangUtils.LANG_OBS_KEY].copy()
            
        #     # make sure to exclude language embedding key that we populate manually
        #     self.obs_keys = tuple([k for k in obs_keys if k != LangUtils.LANG_OBS_KEY])

        # else:
        # if self.use_lang and LangUtils.LANG_COND_ENABLED:
        #     obs_keys += [LangUtils.LANG_OBS_KEY]
        ...
        if self.hdf5_cache_mode in ["all", "low_dim"]:
            ...
            self.hdf5_cache = self.load_dataset_in_memory(demo_list=self.demos,hdf5_file=self.hdf5_file,obs_keys=self.obs_keys_in_memory,dataset_keys=self.dataset_keys,load_next_obs=self.load_next_obs)
    ...
    def load_demo_info(self, filter_by_attribute=None, demos=None):
        ...
        if demos is not None:
            self.demos = demos
        elif filter_by_attribute is not None and not isinstance(filter_by_attribute,list):
        ...
        # remeber to remove it
        if filter_by_attribute is not None and  isinstance(filter_by_attribute,list):
            for k in filter_by_attribute:
                self.demos.remove(f"demo_{k}")
    ...
    def get_item(self, index):
        ...
        if LangUtils.LANG_COND_ENABLED:
            assert LangUtils.LANG_OBS_KEY in meta["obs"].keys()
        # if LangUtils.LANG_COND_ENABLED:
        #     # language embedding
        #     T = meta["actions"].shape[0]
        #     meta["obs"][LangUtils.LANG_OBS_KEY] = np.tile(self._lang_emb, (T, 1))
