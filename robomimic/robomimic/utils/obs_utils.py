...
class LangModality(Modality):
    """
    Modality for lang observations
    """
    name = "lang"
...
class ImageClipModality(Modality):
    """
    Modality for RGB image observations
    """
    name = "cliprgb"
    ...
    def _default_obs_processor(cls, obs):
        ...
        if len(obs.shape)==3:
            return np.array((ImageClipModality.processor(obs/255,do_rescale=False,do_normalize=False).pixel_values)[0])
        return np.array((ImageClipModality.processor(obs/255,do_rescale=False,do_normalize=False).pixel_values))
