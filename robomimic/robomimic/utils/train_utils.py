...
def load_data_for_training(config, obs_keys):
    ...
    if valid_filter_by_attribute is not None:
        assert config.experiment.validate, "specified validation filter key {}, but config.experiment.validate is not set".format(
...
def dataset_factory(config,
    ...
    ds_kwargs = dict(
        ...
        load_next_obs=config.train.
        hdf5_load_next_obs,  # whether to load next observations (s') from dataset
...
def run_rollout(
    ...
    try:
        for step_i in range(horizon):
            ...
            #     ob_dict["frontview_image"] = np.transpose(
            #         ob_dict["frontview_image"], (2, 0, 1))
            # ob_dict["pcd"] = np.zeros((3,2048))
            ...
            if video_writer is not None:
                if video_count % video_skip == 0:
                    video_img = env.render(mode="rgb_array",
                                           height=512,
                                           width=512)
...
def rollout_with_stats(
    ...
    assert (video_path is None) or (
        video_dir
        is None), "rollout_with_stats: can't specify both video path and dir"
    ...
    if video_dir is not None:
        ...
        video_str = "_epoch_{}.mp4".format(
            epoch) if epoch is not None else ".mp4"
        video_paths = {
            k: os.path.join(video_dir, "{}{}".format(k, video_str))
            for k in envs
        }
        video_writers = {
            k: imageio.get_writer(video_paths[k], fps=20)
            for k in envs
        }
    ...
    for env_name, env in envs.items():
        ...
        print("rollout: env={}, horizon={}, use_goals={}, num_episodes={}".
              format(
                  env.name,
                  horizon,
                  use_goals,
        ...
        for ep_i in range(num_episodes):
            ...
            if verbose:
                print("Episode {}, horizon={}, num_success={}".format(
        ...
        rollout_logs_mean = dict(
            (k, np.mean(v)) for k, v in rollout_logs.items())
        rollout_logs_mean["Time_Episode"] = np.sum(
            rollout_logs["time"]
        ) / 60.  # total time taken for rollouts in minutes
...
def save_model(model,
    ...
    if obs_normalization_stats is not None:
        ...
        params["obs_normalization_stats"] = TensorUtils.to_list(
...
def run_epoch(model,
    ...
    step_log_all = []
    timing_stats = dict(Data_Loading=[],
                        Process_Batch=[],
                        Train_Batch=[],
                        Log_Info=[])
    ...
    step_log_all = dict(
        (k, float(np.mean(v))) for k, v in step_log_dict.items())
