...
class MujocoManipulatorEnv(FrankaBase):
    ...
    def reset(self, joint_pos: List[float] = None, joint_vel: List[float] = None):
        ...
        if joint_pos is None:
            self.data.qpos = self.rest_pose
        if joint_vel is not None:
            self.data.qvel = joint_vel
    ...
    def viewer_setup(self):
        if self.has_renderer:
            return mujoco_viewer.MujocoViewer(
                self.model, self.data, height=self.img_height, width=self.img_width, hide_menus=True
            )
    ...
    def _start_custom_controller(self, q_desired=None, ctrl_mode=0.0):
        ...
        self.policy = MixedCartesianImpedanceControl(
            ...
            * torch.tensor(
            ...
            kp_pos=self.gain_scale * torch.nn.Parameter(torch.tensor(self.metadata["default_Kq"], dtype=torch.float32)),
            kd_pos=self.gain_scale * torch.nn.Parameter(torch.tensor(self.metadata["default_Kqd"], dtype=torch.float32)),
    ...
    def adaptive_time_to_go(self, desired_joint_position, t_min=1, t_max=4):
        ...
        time_to_go = self._adaptive_time_to_go_polymetis(self.toco_robot_model, displacement)
        ...
        for i in range(len(waypoints)):
            ...
            for _ in range(int(dt // self.model.opt.timestep)):
                self.update_desired_joint_positions(
                    ...
                    * torch.nn.Parameter(torch.tensor(self.metadata["default_Kq"], dtype=torch.float32)),
                    kd=self.reset_gain_scale
                    * torch.nn.Parameter(torch.tensor(self.metadata["default_Kqd"], dtype=torch.float32)),
                )
            # time.sleep(dt)
