...
from training.weird_diffusion.models.make_networks import instantiate_model_artifacts
...
@hydra.main(
    version_base=None, config_path="../../configs", config_name="diffusion_policy_sim"
)
def run_experiment(cfg):
    ...
    nets, ema, noise_scheduler, optimizer, lr_scheduler, dataloader, stats, device = (
        instantiate_model_artifacts(cfg, model_only=False)
    )
        ...
        for epoch_idx in tglobal:
            ...
            with tqdm(dataloader, desc="Batch", leave=False) as tepoch:
                for nbatch in tepoch:
                    ...
                    image_features = nets["vision_encoder"](nimage.flatten(end_dim=2))
                    ...
                    if cfg.training.with_state:
                        nagent_pos = nbatch["state"][:, : cfg.training.obs_horizon].to(
                            device
                        )
            ...
            if epoch_idx % 1000 == 0:
                ...
                torch.save(
                    checkpoint, os.path.join(logdir, f"diffusion_policy_{epoch_idx}")
                )
    ...
    ema.copy_to(ema_nets.parameters())
    # Create a state dict with the paramaters of the model, and the stats of the dataset
    checkpoint = {"state_dict": ema_nets.state_dict(), "stats": stats, "config": cfg}
