...
# https://github.com/openai/mujoco-worldgen/blob/master/mujoco_worldgen/util/rotation.py#L110
...
def euler_to_quat_mujoco(euler):
...
def quat_conjugate_mujoco(q):
    inv_q = -q
    inv_q[..., 0] *= -1
    return inv_q


def quat_mul_mujoco(quat0, quat1):
...
def quat_rot_vec_mujoco(q, v0):
    q_v0 = np.array([0, v0[0], v0[1], v0[2]])
    q_v = quat_mul_mujoco(q, quat_mul_mujoco(q_v0, quat_conjugate_mujoco(q)))
    v = q_v[1:]
    return v


def quat_identity_mujoco():
    return np.array([1, 0, 0, 0])


def quat_difference_mujoco(q, p):
    return quat_normalize_mujoco(quat_mul_mujoco(q, quat_conjugate_mujoco(p)))


def quat_magnitude_mujoco(q):
    w = q[..., 0]
    assert np.all(w >= 0)
    return 2 * np.arccos(np.clip(w, -1.0, 1.0))


def quat_normalize_mujoco(q):
    assert q.shape[-1] == 4
    return q * np.sign(q[..., [0]])  # use quat with w >= 0


def quat_average_mujoco(quats, weights=None):
    ...
    return quat_normalize_mujoco(evecs[:, biggest_i])


def quat_to_axisangle_mujoco(quat):
    theta = 0
    axis = np.array([0, 0, 1])
...
def euler_to_point_euler_mujoco(euler):
    _euler = euler.copy()
...
def point_euler_to_euler_mujoco(euler):
    _euler = euler.copy()
    if len(_euler.shape) < 2:
        _euler = np.expand_dims(_euler, 0)
...
def quat_to_point_quat_mujoco(quat):
...
def point_quat_to_quat_mujoco(quat):
    _quat = quat.copy()
    if len(_quat.shape) < 2:
        _quat = np.expand_dims(_quat, 0)
...
def normalize_angles_mujoco(angles):
...
def round_to_straight_angles_mujoco(angles):
    """Returns closest angle modulo 90 degrees"""
    angles = np.round(angles / (np.pi / 2)) * (np.pi / 2)
    return normalize_angles_mujoco(angles)


def round_to_straight_quat_mujoco(quat):
    angles = quat_to_euler_mujoco(quat)
    rounded_angles = round_to_straight_angles_mujoco(angles)
    return euler_to_quat_mujoco(rounded_angles)


def get_parallel_rotations_mujoco():
    mult90 = [0, np.pi / 2, -np.pi / 2, np.pi]
    parallel_rotations = []
    for euler in itertools.product(mult90, repeat=3):
        canonical = mat_to_euler_mujoco(euler_to_mat_mujoco(euler))
...
def quat_from_angle_and_axis_mujoco(angle, axis):
...
def uniform_quat_mujoco(random):
    ...
    """
    w = random.randn(4)
    return quat_normalize_mujoco(w / np.linalg.norm(w))


def apply_euler_rotations_mujoco(base_quat, rotation_angles):
    """Apply a sequence of euler angle rotations on to the base quaternion"""
    new_rot_mat = np.eye(3)
    for rot_angle in rotation_angles:
        new_rot_mat = np.matmul(
            euler_to_mat_mujoco(rot_angle * np.pi / 2.0), new_rot_mat
        )

    new_rot_mat = np.matmul(quat_to_mat_mujoco(base_quat), new_rot_mat)
    new_quat = mat_to_quat_mujoco(new_rot_mat)
    return new_quat
