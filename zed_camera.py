...
class ZedCamera:
    ...
    def read_camera(self):
        ...
        # Read Camera #
        start = time.time()
        timestamp_dict = {self.serial_number + "_read_start": start}
        err = self._cam.grab(self._runtime)
        if err != sl.ERROR_CODE.SUCCESS:
            return None
        timestamp_dict[self.serial_number + "_read_end"] = time.time()-start
        ...
        img_right = imgs[str(self.serial_number) + "_right"]
        
        # The memory layout of color channels in LEFT mode is not RGBA, but BGRA.
        # https://github.com/stereolabs/zed-opencv/issues/72#issuecomment-723973645
        img_right_tmp = img_right.copy()
        img_right[:,:,0] = img_right_tmp[:,:,2]
        img_right[:,:,2] = img_right_tmp[:,:,0]

        dict_1 = {'color_image': img_right, 'shape': img_right.shape, 'type': 'rgb',
                  'read_time': timestamp_dict[str(self.serial_number) + "_estimated_capture"], 'serial_number': self.serial_number + '/rgb'}

        return [dict_1, dict_1]
